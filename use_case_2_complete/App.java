        ...
        String schemaJson = """
{
        ...
        try (ClientEncryption clientEncryption = this.getClientEncryption(connectionString, keyvaultNamespace, kmsProvider)) {
            ...
            if (employeeKeyDoc == null) {
                ...
                masterKey.append("keyId", new BsonString(keyId))
                    .append("endpoint", new BsonString(String.format("csfle-kmip-%s.mdbps.internal", STUDENTNAME)));
                DataKeyOptions dataKeyOptions = new DataKeyOptions()
                    .masterKey(masterKey)
                    .keyAltNames(Arrays.asList(altName));
                clientEncryption.createDataKey(provider, dataKeyOptions).subscribe(newKeySubscriber);
                BsonBinary bsonKey = newKeySubscriber.first();
    ...
    public static void main( String[] args )
    {
        ...
        try {
            ...
            extraOptions.put("cryptSharedLibPath", "/home/ubuntu/crypt_shared/lib/mongo_crypt_v1.so");
            ...
            try (MongoClient secureClient = app.getMdbClient(connectionString, 5000, false, autoEncryptionSettings)) {
                ...
                MongoDatabase keyvaultDatabase = client.getDatabase(keyvaultNamespace.getDatabaseName());
                MongoCollection<Document> keyvaultCollection = keyvaultDatabase.getCollection(keyvaultNamespace.getCollectionName());
                ObservableSubscriber<DeleteResult> delSubscriber = new OperationSubscriber<DeleteResult>();
                keyvaultCollection
                    .deleteOne(eq("keyAltNames", employeeId))
                    .subscribe(delSubscriber);
                delSubscriber.await();
